...
def pupils_set_func():
    """Присвоение глобальной переменной значения radio button"""
    global learning_moment
...
def visible_reg_window_func():
    ...
    # Всё, что показываем
    reg_window.show()  # Показ окна регистрации
    # Всё, что скрываем
    login_window.hide()
    main_menu.hide()
...
def visible_app_window():
    ...
    login_window.hide()
    app_window.show()
# Вызов класса из другого файла
db = SQLighter('db.db')

# Создание приложения
app = QtWidgets.QApplication([])

# Созадние окна регистрации
reg_window = uic.loadUi("Reg.ui")
# Подключение икноки к окну
